...
function ProjectCard({ title, stack, desc, color, link }) {
  const styledSquare = [
  ...
  return (
    <a href={link} target="_blank" rel="noopener noreferrer">
      <div className="border flex hover:border-gray-700 place-items-center cursor-pointer hover:shadow-lg hover:-translate-y-1 duration-200 border-gray-200 mb-5 bg-white group rounded-lg py-3 px-4 relative">
        ...
        <div className="w-5/6">
          <div className="md:flex place-items-center mb-1">
            <h1 className="text-md font-semibold text-blue-700 mb-1 md:mb-0 mr-2">
              {title}
            </h1>
            <div className="flex items-center flex-wrap gap-y-2 gap-x-1">
              {stack.map((item, idx) => (
                <div
                  key={idx}
                  className="px-2 py-0.5 rounded-md bg-blue-200 bg-opacity-40 text-xx"
                >
                  {item}
                </div>
              ))}
          ...
          <p className="text-gray-500 text-xs md:text-xs mb-1 line-clamp-2">
            {desc}
          </p>
        </div>
      </div>
